...
public partial class Views_Shared_OutOfDateSuggestions : System.Web.UI.UserControl
{
    ...
    public void LoadSuggestions()
    {
        if (OrganizationID <= 0)
        {
            ...
            List<SuggestionRouting> list = sr.GetOutOfDateSuggestions().ToList();
            grid_items.DataSource = list;
            grid_items.DataBind();
            //////////////////////
            if (list.Count() > 0)
            {
        ...
        {
            SuggestionsRepository sr = new SuggestionsRepository();
            List<SuggestionRouting> list = sr.GetOutOfDateSuggestions(OrganizationID).ToList();
            grid_items.DataSource = list;
            grid_items.DataBind();
            //////////////////////
            if (list.Count() > 0)
