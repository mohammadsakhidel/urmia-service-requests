<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Operators.ascx.cs" Inherits="Views_Shared_Operators" %>

<% if (Permissions.UsersWritePermission)
   { %>

<div class="grid_tools">
    <a class="btn_new" href='<%= MyHelper.Url("~/Views/" + MyRoles.FolderName + "/ToolWindows/NewOperator.aspx") %>' onclick="return Open('<%= MyHelper.Url("~/Views/" + MyRoles.FolderName + "/ToolWindows/NewOperator.aspx") %>', '', 600, 300);"></a>
</div>
...
<asp:Panel ID="pnl_items" runat="server" style="margin-top : 10px;">
    ...
    <asp:GridView ID="grid_items" runat="server" Width="100%" AllowPaging="true"
        ...
        <Columns>
            ...
            <asp:BoundField HeaderText="نام اپراتور" DataField="Name" ItemStyle-CssClass="grid_item" SortExpression="Name"/>
            ...
            <asp:TemplateField ItemStyle-CssClass="grid_item">
                ...
                <% if (Permissions.UsersWritePermission)
                   ...
                   <a class="grid_button grid_sms" style="margin-left: 5px;" onclick="return Open();"></a>
                    <%# Eval("HtmlOfEditButton") %>
                    <span style="margin-right : 5px;"><%# Eval("HtmlOfAccessRule") %></span>
