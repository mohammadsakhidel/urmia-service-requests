<%@ Control Language="C#" AutoEventWireup="true" CodeFile="Patterns.ascx.cs" Inherits="Views_Shared_Patterns" %>

<% if (Permissions.SendSmsWritePermission)
   { %>

<div class="grid_tools" style="margin : 10px 0 10px 0;">
    <a class="btn_new" href='<%= MyHelper.Url("~/Views/" + MyRoles.FolderName + "/ToolWindows/NewPattern.aspx") %>' onclick="return Open('<%= MyHelper.Url("~/Views/" + MyRoles.FolderName + "/ToolWindows/NewPattern.aspx") %>', '', 600, 350);"></a>
</div>
...
<asp:Panel ID="pnl_items" runat="server">
    ...
    <asp:GridView ID="grid_items" runat="server" Width="100%" AllowPaging="true"
        ...
        <Columns>
            ...
            <asp:TemplateField HeaderText="عنوان الگو" ItemStyle-CssClass="grid_item">
                <ItemTemplate>
                    <span title='<%# Eval("Text") %>'><%# Eval("Name") %></span>
                </ItemTemplate>
            </asp:TemplateField>
                    ...
                    <% if (Permissions.SendSmsWritePermission)
                       { %>
                        ...
                        <asp:LinkButton ID="btn_delete" runat="server" CssClass="grid_button grid_delete" style="margin-right : 3px;"
                            CommandName="DeleteItem" CommandArgument='<%# Eval("ID") %>' ToolTip="حذف" OnClientClick="return confirm('آیا اطمینان دارید؟');">
                        </asp:LinkButton>
